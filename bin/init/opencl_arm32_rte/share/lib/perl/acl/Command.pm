...
sub version {
   my ($self,@args) = @_;
   if ( $#args < 0 ) {
      my $banner = acl::Env::is_sdk() ? 'Altera SDK for OpenCL, Version 16.0.0 Build 206, Copyright (C) 2016 Altera Corporation' : 'Altera Runtime Environment for OpenCL, Version 16.0.0 Build 206, Copyright (C) 2016 Altera Corporation';
      print $self->prog." 16.0.0.206 ($banner)\n";
   } else {
...
sub analyze_area {
   ...
   if ( $html_out =~ /Recompile with -g for detailed area breakdown by source line/ ) {
      print "$input was not compiled with -g. Recompile with -g for detailed area breakdown.\n";
   }
   ...
   print "Area report successfully created: $output_name\n";
   return $self;
}
...
$prog analyze-area - Generate an HTML table containing area usage
                     by source code line. 
...
Description:
   ...
   The aoco or aocx file must be compiled with the -g flag.
...
General:
   ...
   $prog analyze-area      Generate an HTML table containing the area report.
   $prog vis               Launch memory visualizer.
