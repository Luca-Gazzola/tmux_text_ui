...
sub migrate_platform_preqsys {
  ...
  if ( $platform{name} eq 'a10_ref' ) { 
    ...
    # a10_ref 15.1 BSP is not forward compatible with ACDS 16.0+
    if ( $platform{version} eq 15.1 and $quartus_version >= 16.0 ) {
      my $error_string = "Error: a10_ref $platform{version} BSP is not compatible with your version of Quartus ($quartus_version)!\n";
