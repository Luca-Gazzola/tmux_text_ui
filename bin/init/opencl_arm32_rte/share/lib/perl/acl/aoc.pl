...
sub check_env {
  ...
  my $q_pro = 0;
  my $is_prime = 0;
  my $req_qversion_str = exists($ENV{ACL_ACDS_VERSION_OVERRIDE}) ? $ENV{ACL_ACDS_VERSION_OVERRIDE} : "16.0.0";
...
#    }
  ...
  if (($is_prime == 1) && ($q_pro != 1) && ($platform_type =~ /^a10/)) {
    print STDERR "$prog: This release of Altera SDK for OpenCL on A10 requires Quartus Prime Pro Edition.";
    print STDERR " However, the following version was found: \n$q_out\n";
    exit 1;
  }
  if (($is_prime == 1) && ($q_pro == 1) && ($platform_type !~ /^a10/)) {
...
';
  ...
  if ( $run_clang ) {
    ...
    my $fpga_triple = $big_endian ? 'fpga64be' : 'fpga64';
    ...
    if ( not $c_acceleration ) {
      ...
      foreach my $emu_flow (@emu_list) {        
        foreach my $cur_triple (@triple_list) {
          ...
          my @clang_std_opts = ( $emu_flow ? qw(-cc1 -target-abi opencl -emit-llvm-bc -mllvm -gen-efi-tb -DALTERA_CL -Wuninitialized) : qw( -cc1 -O3 -emit-llvm-bc -DALTERA_CL -Wuninitialized));
    ...
    if ($emulator_flow) {
      ...
      $return_status = mysystem_full(
	        ...
	        "$opt_exe -translate-library-calls -reverse-library-translation -lowerconv -insert-ip-library-calls -createemulatorwrapper $emulator_efi_option -generateemulatorsysdesc  $llvm_board_option $llvm_efi_option $llvm_library_option $debug_option $opt_arg_after \"$optinfile\" -o \"$base.bc\" >>$fulllog 2>opt.err" );
  ...
  if ( $run_opt_vfabric ) {
    ...
    if ( $run_vfabric_cfg_gen ) {
      ...
      if ( $generate_vfabric ) {
        ...
        save_vfabric_files_to_pkg($pkg, $var_id, $vfab_lib_path, $work_dir);
  ...
  while ($restart_acl) { # Might have to restart with lmem replication disabled
    ...
    if ( $run_verilog_gen ) {
      ...
      if (-e $area_rpt_file_path) {
        ...
        while ( <$area_rpt_file> ) {
          my $line = $_;
          if ( $line =~ m/\"ram_util_%\":(\d+).(\d+)/ ) {  # the first line that matches is the utilization of the overall system
            if ( $1 > $max_mem_percent_with_replication && !$disabled_lmem_replication ) {
              ...
              if ( $repl_factor_active ) {
                ...
                last; # Break out or else the above message may be printed multiple times if individual kernel mem utilization is above 100%
              }
            }
          }
        }
        ...
        if ($griffin_flow)
        {
          # Griffin flow presently does not produce the area.json file, so do not rerun the loop.
          $restart_without_lmem_replication = 0;
        }
        else
        {
  ...
  if ( $run_verilog_gen && !$vfabric_flow) { 
    ...
    while (($util == -999 or $ffs == -999 or $rams == -999 or $dsps == -999) and 
      ...
      if ($json_line =~ m/\"le_util_%\":(\d+(.\d+)?)/) {  
        $les = $1;
      } elsif ($json_line =~ m/\"ff_util_%\":(\d+(.\d+)?)/) {  
        $ffs = $1;
...
sub version($) {
  ...
  print $outfile "Version 16.0.0 Build 206\n";
...
sub add_hash_sections($$$$) {
  ...
  my $ftemp;
  my $random_hash_key;
  open(my $ftemp, '>', $temp_hashed_file) or die "Could not open file $!";
  my $rand_key = rand;
